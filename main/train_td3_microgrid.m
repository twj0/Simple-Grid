function [agent, training_stats] = train_td3_microgrid(varargin)
% TRAIN_TD3_MICROGRID - Main training script for TD3 microgrid agent
% ?????TD3??????????????
%
% This script performs the complete training workflow for TD3 algorithm:
...
%% === Initialize ===
fprintf('=== TD3 Microgrid Training Started ===\n');
fprintf('Time: %s\n', char(datetime('now')));
...
try
    ...
    training_cfg = training_config_td3();
    fprintf('? TD3 training configuration loaded\n');
...
%% === Step 4: Create TD3 Agent ===
fprintf('\n--- Step 4: Creating TD3 Agent ---\n');
try
    % Create TD3 agent
    agent = create_td3_agent(obs_info, action_info, training_cfg);
    fprintf('? TD3 agent created successfully\n');
    
catch ME
    error('Failed to create TD3 agent: %s', ME.message);
end
...
if save_results
    ...
    try
        ...
        filename = sprintf('%strained_td3_agent_%s.mat', save_dir, timestamp);
        ...
        % Also save as latest
        latest_filename = sprintf('%slatest_td3_agent.mat', save_dir);
